...
対象になるのは、実行時に選択しているシート（タブ）1枚だけです。複数のタブがあっても、他のタブは読み込まれません。別のタブの辞書を生成したい場合は、そのタブに切り替えてからもう一度実行してください。

生成されるファイル名にはそのシート名が使われます。シート名が`example`なら`example-googleime.txt`のようになります。
